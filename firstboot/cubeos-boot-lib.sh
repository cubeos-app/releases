...
write_netplan_for_mode() {
    ...
    case "$mode" in
        offline_hotspot)
            # offline_hotspot: Air-gapped operation. wlan0 (AP) serves the 10.42.24.0/24
...
NETPLAN_ONLINE_ETH
        ...
        wifi_bridge)
            # wifi_bridge: AP mode with internet via USB WiFi dongle (wlan1).
            if [ -z "$wifi_ssid" ]; then
...
# CubeOS Network — wifi_bridge (AP + NAT via USB WiFi dongle wlan1)
# Auto-generated by apply_network_mode — do not edit manually.
# wlan0 MUST be under ethernets (NOT wifis) — hostapd manages the radio.
...
NETPLAN_SERVER_ETH
        ...
        wifi_client)
            # wifi_client: No AP, wlan0 as WiFi client only.
            if [ -z "$wifi_ssid" ]; then
            ...
            if [ "${NET_USE_STATIC_IP:-0}" = "1" ] && [ -n "${NET_STATIC_IP:-}" ] && [ -n "${NET_STATIC_GATEWAY:-}" ]; then
                ...
                {
                    echo "# CubeOS Network — wifi_client (no AP, wlan0 static IP)"
                    echo "# Auto-generated by apply_network_mode — do not edit manually."
                    echo "network:"
                    echo "  version: 2"
                    echo "  renderer: networkd"
...
# CubeOS Network — wifi_client (no AP, wlan0 as WiFi client)
# Auto-generated by apply_network_mode — do not edit manually.
network:
  version: 2
  renderer: networkd
...
configure_pihole_dhcp() {
    ...
    case "$mode" in
        ...
        wifi_router)
            ...
            log_ok "Pi-hole DHCP: active, no-dhcp-interface=eth0 (wifi_router)"
            ;;
        wifi_bridge)
            docker exec cubeos-pihole pihole-FTL --config dhcp.active true 2>/dev/null || true
            ...
            log_ok "Pi-hole DHCP: active, no-dhcp-interface=wlan1 (wifi_bridge)"
            ;;
...
apply_network_mode() {
    ...
    case "$NET_MODE" in
        wifi_router)
            log_ok "Mode: wifi_router -- NAT via eth0"
        ...
        wifi_bridge)
            log_ok "Mode: wifi_bridge -- NAT via wlan1"
            ...
            if ip addr show wlan1 2>/dev/null | grep -q "inet "; then
                ...
                log_ok "wifi_bridge: wlan1 connected ($WLAN1_IP)"
            else
        ...
        eth_client)
            log_ok "Mode: eth_client -- no AP, eth0 DHCP"
        ...
        wifi_client)
            log_ok "Mode: wifi_client -- no AP, wlan0 WiFi client"
