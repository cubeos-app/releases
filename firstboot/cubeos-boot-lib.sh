...
#     - OFFLINE: eth0 static 10.42.24.1 (not DHCP), wlan0 static AP
...
#   - OFFLINE:      eth0 static 10.42.24.1 (wired clients), wlan0 AP
#   - ONLINE_ETH:   eth0 DHCP upstream, wlan0 AP
#   - ONLINE_WIFI:  wlan1 WiFi client upstream, wlan0 AP (no eth0)
#   - SERVER_ETH:   eth0 DHCP only, no AP
#   - SERVER_WIFI:  wlan0 WiFi client only, no AP
write_netplan_for_mode() {
    ...
    case "$mode" in
        offline)
            # OFFLINE: Air-gapped operation. Both wlan0 (AP) and eth0 serve
            # the 10.42.24.0/24 subnet. eth0 gets a static IP so wired clients
            # can connect directly — no DHCP client on eth0 (nothing upstream).
            cat > "$NETPLAN_FILE" << 'NETPLAN_OFFLINE'
# CubeOS Network — OFFLINE (air-gapped, AP + wired clients)
...
# eth0 has static IP — no DHCP client (nothing upstream in offline mode).
network:
  version: 2
  renderer: networkd
  ethernets:
...
NETPLAN_OFFLINE
        ...
        online_eth)
            ...
            # eth0 is a DHCP client on the upstream network.
            # wlan0 is the AP serving 10.42.24.0/24.
            # use-dns: false prevents upstream DHCP from overriding Pi-hole.
...
NETPLAN_ONLINE_ETH
        ...
        online_wifi)
            ...
            # wlan1 connects as a WiFi client to the upstream network.
            # wlan0 is the AP serving 10.42.24.0/24.
            # eth0 is NOT configured — not used in this mode.
            if [ -z "$wifi_ssid" ]; then
...
NETPLAN_ONLINE_WIFI
        ...
        server_eth)
            # SERVER_ETH: No AP, Ethernet DHCP client only.
            # CubeOS joins an existing network as a regular host.
...
NETPLAN_SERVER_ETH
        ...
        server_wifi)
            ...
            # CubeOS joins an existing WiFi network as a regular host.
            if [ -z "$wifi_ssid" ]; then
