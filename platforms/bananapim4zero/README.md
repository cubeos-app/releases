...
> The M4 Zero has WiFi AP mode configured via hostapd (same as Raspberry Pi).
...
- WiFi AP mode (RTL8821CS + hostapd) — configured, needs testing on real hardware
...
- **Networking:** WiFi AP (hostapd + RTL8821CS) + wired Ethernet (same modes as Pi)
...
| WiFi | Yes (hostapd AP) | No | Yes (hostapd AP, needs HW test) |
| eMMC | No | No | 32GB |
